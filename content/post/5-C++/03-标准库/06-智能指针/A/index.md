...
# 1. shared_ptr
共享智能指针，内部维护一块内存，允许多个shared\_ptr共享这块内存。   
```cpp
// Defined in header <memory>
template< class T > class shared_ptr;
```
shared\_ptr内部通过`引用计数`来管理对象的生命周期，当引用计数为0时，对象会被自动销毁。    
```cpp
// int* x = new int(10);
...
2. get方法，返回原始指针，，但是要注意这个操作不会增加引用计数，所以不要用它来初始化另一个shared_ptr, 或者delete它，因为shared_ptr并不知道你干了什么，所以会导致多次delete。   
3. reset()方法，先将引用计数减1，然后判断是否为0，如果为0，就则释放管理的原始内存。  
4. `template< class Y > void reset( Y* ptr );`，先将引用计数减1，然后判断是否为0，如果为0，就释放管理的原始内存，并将ptr赋值给内部的指针。如果不为0，那就什么也不做。   
